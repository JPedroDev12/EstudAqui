...
export async function criar(req, res) {
  const { categoria_id, nome, modalidade, descricao, capa, usuario_id } = req.body;

  if (!categoria_id || !nome || !modalidade) {
    return res.status(400).json({ erro: 'Campos obrigatórios: categoria_id, nome e modalidade' });
  }
  ...
  try {
    const [categoriaExiste] = await conexao.query('SELECT id FROM categorias WHERE id = ?', [categoria_id]);
    if (categoriaExiste.length === 0) {
      return res.status(404).json({ erro: 'Categoria não encontrada' });
    }

    const [result] = await conexao.query(
      `INSERT INTO cursos (usuario_id, categoria_id, nome, modalidade, descricao, capa, ativo) VALUES (?, ?, ?, ?, ?, ?, 1)`,
...
export async function atualizar(req, res) {
  ...
  if (!id || isNaN(Number(id))) return res.status(400).json({ erro: 'ID inválido' });
  if (!categoria_id || !nome || !modalidade) {
    return res.status(400).json({ erro: 'Campos obrigatórios: categoria_id, nome e modalidade' });
  }
  if (!['Presencial', 'Online'].includes(modalidade)) {
  ...
  try {
    ...
    await conexao.query(
      `UPDATE cursos SET usuario_id = ?, categoria_id = ?, nome = ?, modalidade = ?, descricao = ?, capa = ? WHERE id = ?`,
      [
        usuario_id,
        categoria_id,
        nome,
    ...
    res
      .status(500)
      .json({ erro: "Erro ao atualizar curso", detalhe: err.message });
  }
}
...
export async function remover(req, res) {
  ...
  try {
    ...
    if (cursoExiste.length === 0) return res.status(404).json({ erro: 'Curso não encontrado' });
    if (cursoExiste[0].ativo === 0) return res.status(409).json({ erro: 'Curso já está inativo' });
